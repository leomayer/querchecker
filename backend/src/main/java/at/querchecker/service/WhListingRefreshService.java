...
public class WhListingRefreshService {
  ...
  public void refreshAsync(Long listingId, Long whItemId) {
    ...
    // Update image paths
    if (
      advert.getAdvertImageList() != null &&
      ...
      List<String> freshPaths = images
        .stream()
        .filter(
          (img) ->
            img.getMainImageUrl() != null &&
    ...
    if (listing.getWhCategory() == null) {
      ...
      if (catWhId != null) {
        ...
        if (cat != null) {
          ...
          changed = true;
          log.debug(
            "Lazy category assigned async: listingWhId={} → {}",
            listing.getWhId(),
            cat.getName()
          );
        }
    ...
    sseHub.broadcast(
      "listing-refreshed",
      ListingRefreshedPayload.builder()
        .whItemId(whItemId)
