...
public final class CategorySearchSourceDefinitions {
    ...
    public record SourceConfig(
            String domain, String label, SourceType type,
            boolean lookupEnabled,
            List<String> queryExcludes, int searchResultCount) {}

    private static final int SNIPPETS_COUNT   = 10; // alle Snippets ans LLM
    private static final int HTML_FETCH_COUNT =  3; // Top-URL + 2 Fallbacks
    ...
    public static final Map<String, List<SourceConfig>> CONFIGS = Map.ofEntries(
            ...
            entry("Drucker", List.of(
                    new SourceConfig("icecat.biz", "Icecat", ICECAT, true, ICECAT_EXCLUDES, SNIPPETS_COUNT))),

            entry("PC-Komponenten", List.of(          // level=1 → Grafikkarten, CPUs etc. erben
                    new SourceConfig("icecat.biz", "Icecat", ICECAT, true, ICECAT_EXCLUDES, SNIPPETS_COUNT))),

            entry("Netzwerke", List.of(               // level=1 → Server, Router etc. erben
                    new SourceConfig("icecat.biz", "Icecat", ICECAT, true, ICECAT_EXCLUDES, SNIPPETS_COUNT))),

            entry("Kameras / Camcorder", List.of(     // level=1 → Digitalkameras, Systemkameras etc. erben
                    new SourceConfig("icecat.biz", "Icecat", ICECAT, true, ICECAT_EXCLUDES, SNIPPETS_COUNT))),
            ...
            entry("Adapter / Kabel", List.of(         // level=1
                    new SourceConfig("icecat.biz", "Icecat", ICECAT, true, ICECAT_EXCLUDES, SNIPPETS_COUNT))),

            entry("Software", List.of(                // level=1
                    new SourceConfig("icecat.biz", "Icecat", ICECAT, true, ICECAT_EXCLUDES, SNIPPETS_COUNT))),

            entry("Spiele", List.of(                  // level=1 → PS/Xbox/PC-Spiele erben
                    new SourceConfig("icecat.biz", "Icecat", ICECAT, true, ICECAT_EXCLUDES, SNIPPETS_COUNT)))
    );
}
